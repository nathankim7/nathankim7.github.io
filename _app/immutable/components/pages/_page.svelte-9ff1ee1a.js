import { S as SvelteComponent, i as init, s as safe_not_equal, J as add_render_callback, k as element, l as claim_element, m as children, h as detach, n as attr, b as insert_hydration, K as add_resize_listener, L as listen, C as noop, o as onMount, M as globals, v as binding_callbacks, a as space, x as create_component, q as text, N as head_selector, c as claim_space, y as claim_component, r as claim_text, p as set_style, O as src_url_equal, H as append_hydration, z as mount_component, f as transition_in, t as transition_out, A as destroy_component } from "../../chunks/index-83ca9dda.js";
...
var Children = class extends Collection {
  constructor(children2) {
    children2 = Array.isArray(children2) ? children2 : Array.prototype.slice.call(arguments);
    super(children2);
    __publicField(this, "ids", {});
    this.attach(children2);
  ...
  detach(children2) {
    for (let i = 0; i < children2.length; i++) {
      delete this.ids[children2[i].id];
    }
    return this;
  }
};
...
var _Group = class extends Shape {
  constructor(children2) {
    ...
    this.additions = [];
    this.subtractions = [];
    this.children = Array.isArray(children2) ? children2 : Array.prototype.slice.call(arguments);
  }
  static InsertChildren(children2) {
    for (let i = 0; i < children2.length; i++) {
      replaceParent.call(this, children2[i], this);
    }
  }
  static RemoveChildren(children2) {
    for (let i = 0; i < children2.length; i++) {
      replaceParent.call(this, children2[i]);
    }
  }
  static OrderChildren(children2) {
    this._flagOrder = true;
  }
  clone(parent) {
    const clone = new _Group();
    const children2 = this.children.map(function(child) {
      return child.clone();
    });
    clone.add(children2);
...
var proto5 = {
  ...
  children: {
    ...
    set: function(children2) {
      ...
      this._children = new Children(children2);
...
var dom = {
  ...
  getRequestAnimationFrame: function() {
    ...
    function fallbackRequest(callback, element2) {
...
function extractCSSText(text2, styles) {
  if (!styles) {
    styles = {};
  }
  const commands = text2.split(";");
...
function applySvgAttributes(node, elem, parentStyles) {
  ...
  for (i = 0; i < node.attributes.length; i++) {
    attr2 = node.attributes[i];
    if (/style/i.test(attr2.nodeName)) {
      extractCSSText(attr2.value, extracted);
    } else {
      attributes[attr2.nodeName] = attr2.value;
    }
  }
...
var read = {
  ...
  use: function(node, styles) {
    ...
    for (let i = 0; i < node.attributes.length; i++) {
      const attr2 = node.attributes[i];
      const ca = overwriteAttrs.includes(attr2.nodeName);
      const cb = !fullNode.hasAttribute(attr2.nodeName);
      if (ca || cb) {
        fullNode.setAttribute(attr2.nodeName, attr2.value);
      }
    }
  ...
  text: function(node, parentStyles) {
    ...
    applySvgAttributes.call(this, node, text2, parentStyles);
    text2.alignment = alignment;
    text2.baseline = baseline;
    return text2;
  },
...
var _Two = class {
  ...
  makeText(message, x, y, styles) {
    const text2 = new Text(message, x, y, styles);
    this.add(text2);
    return text2;
  }
...
function create_fragment$1(ctx) {
  ...
  return {
    ...
    l(nodes) {
      div_1 = claim_element(nodes, "DIV", { id: true, class: true });
      children(div_1).forEach(detach);
      this.h();
    },
    h() {
      attr(div_1, "id", "bg");
      attr(div_1, "class", "w-100 h-100 ants");
      add_render_callback(() => (
    ...
    m(target, anchor2) {
      insert_hydration(target, div_1, anchor2);
      ctx[4](div_1);
      div_1_resize_listener = add_resize_listener(
        div_1,
        /*div_1_elementresize_handler*/
        ctx[5].bind(div_1)
      );
...
function instance($$self, $$props, $$invalidate) {
  ...
  const Point = (x, y, r, c) => {
    ...
    self2.sprite = two.makeCircle(self2.x, self2.y, self2.radius);
    self2.sprite.noStroke();
    self2.sprite.fill = self2.colour;
    return self2;
  };
  const Animatable = () => {
    return { flags: { remove: false } };
  };
  const Ant = (path, hill, p) => {
    ...
    const behaviours = (self3) => ({
      move: () => {
        let closest = closestHole(self3.x, self3.y, 20);
        if (closest >= 0) {
          let hole = radialGrid[closest / radialGrid[0].length | 0][closest % radialGrid[0].length];
          if (closest != radialGridLookUp(self3.path.start.x, self3.path.start.y) && hole.polarity != self3.p) {
            self3.flags.remove = true;
            self3.sprite.remove();
            hole.flip();
            if (Math.random() < 0.1)
              paths.push(Path2(hole, Math.random() * 0.35 + 0.4, PARTY_SIZE));
            return;
          }
        }
        if (self3.ind == self3.path.num) {
          self3.path.make(self3.x, self3.y, self3.path.moves[self3.ind - 1].d);
        }
        let move = self3.path.moves[self3.ind];
        if (move.type == "curve") {
          self3.progress += ANT_V / move.r;
          let a = self3.progress;
          self3.x = move.c.x + move.r * Math.cos(move.turn * a + (move.d + 2) * Math.PI / 2);
          self3.y = move.c.y - move.r * Math.sin(move.turn * a + (move.d + 2) * Math.PI / 2);
        } else if (move.type == "straight") {
          self3.progress += ANT_V;
          self3.x += (move.d > 1 ? -1 : 1) * ((move.d + 1) % 2) * ANT_V;
          self3.y += (move.d > 1 ? 1 : -1) * (move.d % 2) * ANT_V;
        }
        self3.x = (self3.x - offsetX + canvasW) % canvasW + offsetX;
        self3.y = (self3.y - offsetY + canvasH) % canvasH + offsetY;
        self3.render();
        if (self3.progress > (move.type == "curve" ? Math.PI / 2 : move.l)) {
          self3.ind++;
          self3.progress = 0;
        }
      }
    });
    return { ...self2, ...behaviours(self2) };
  };
  const Path2 = (startHole, straightness, size) => {
    const self2 = {
      start: startHole,
      straightness,
      size,
    ...
    self2.make(self2.start.x, self2.start.y, Math.random() * 4 | 0);
    return self2;
  };
  let div;
  const run = () => {
...
class Ants extends SvelteComponent {
  constructor(options) {
    super();
...
function create_fragment(ctx) {
  ...
  return {
    c() {
      ...
      t0 = space();
      div1 = element("div");
      create_component(ants.$$.fragment);
      t1 = space();
      div0 = element("div");
      p0 = element("p");
      t2 = text("NATHAN KIM");
      ...
      t4 = space();
      span = element("span");
      t5 = text("Hi! I'm a sophomore student at Stanford University interested in\n			");
      p1 = element("p");
      t6 = text("computers");
      t7 = text("\n			and\n			");
      p2 = element("p");
      ...
      t9 = text("\n			and especially\n			");
      p3 = element("p");
      t10 = text("things that combine the two");
      t11 = text("\n			. Intermittently, I've also been a full-stack web dev and an Android developer. Feel free to check\n			out my code on");
      a0 = element("a");
      t12 = text("Github");
      t13 = text(", connect with me on\n			");
      a1 = element("a");
      t14 = text("Linkedin");
      t15 = text(", browse my\n			");
      a2 = element("a");
      t16 = text("publications");
      t17 = text(", and\n			take my\n			");
      a3 = element("a");
      t18 = text("resume");
      t19 = text(". Any questions or comments can be\n			directed to me by filling in the blanks at\n			");
      a4 = element("a");
      t20 = text("nathangk@st_nf_rd.edu");
      t21 = text(".");
      t22 = space();
      p4 = element("p");
      t23 = text("things that might interest you");
      t24 = space();
      hr1 = element("hr");
      t25 = space();
      a5 = element("a");
      t26 = text("[Web App] Odd One Out: A Game of Words in Vector Semantics");
      this.h();
    },
    l(nodes) {
      const head_nodes = head_selector("svelte-e8wxlh", document.head);
      link = claim_element(head_nodes, "LINK", { rel: true, href: true });
      head_nodes.forEach(detach);
      t0 = claim_space(nodes);
      div1 = claim_element(nodes, "DIV", { class: true });
      var div1_nodes = children(div1);
      claim_component(ants.$$.fragment, div1_nodes);
      t1 = claim_space(div1_nodes);
      div0 = claim_element(div1_nodes, "DIV", { id: true, class: true });
      var div0_nodes = children(div0);
      p0 = claim_element(div0_nodes, "P", { id: true, class: true });
      var p0_nodes = children(p0);
      t2 = claim_text(p0_nodes, "NATHAN KIM");
      ...
      hr0 = claim_element(div0_nodes, "HR", { class: true, style: true });
      t4 = claim_space(div0_nodes);
      span = claim_element(div0_nodes, "SPAN", { style: true, class: true });
      var span_nodes = children(span);
      t5 = claim_text(span_nodes, "Hi! I'm a sophomore student at Stanford University interested in\n			");
      p1 = claim_element(span_nodes, "P", { class: true });
      var p1_nodes = children(p1);
      t6 = claim_text(p1_nodes, "computers");
      p1_nodes.forEach(detach);
      t7 = claim_text(span_nodes, "\n			and\n			");
      p2 = claim_element(span_nodes, "P", { class: true });
      var p2_nodes = children(p2);
      t8 = claim_text(p2_nodes, "language");
      p2_nodes.forEach(detach);
      t9 = claim_text(span_nodes, "\n			and especially\n			");
      p3 = claim_element(span_nodes, "P", { class: true });
      var p3_nodes = children(p3);
      t10 = claim_text(p3_nodes, "things that combine the two");
      p3_nodes.forEach(detach);
      t11 = claim_text(span_nodes, "\n			. Intermittently, I've also been a full-stack web dev and an Android developer. Feel free to check\n			out my code on");
      a0 = claim_element(span_nodes, "A", { href: true, class: true });
      var a0_nodes = children(a0);
      t12 = claim_text(a0_nodes, "Github");
      a0_nodes.forEach(detach);
      t13 = claim_text(span_nodes, ", connect with me on\n			");
      a1 = claim_element(span_nodes, "A", { href: true, class: true });
      var a1_nodes = children(a1);
      t14 = claim_text(a1_nodes, "Linkedin");
      a1_nodes.forEach(detach);
      t15 = claim_text(span_nodes, ", browse my\n			");
      a2 = claim_element(span_nodes, "A", { href: true, class: true });
      var a2_nodes = children(a2);
      t16 = claim_text(a2_nodes, "publications");
      a2_nodes.forEach(detach);
      t17 = claim_text(span_nodes, ", and\n			take my\n			");
      a3 = claim_element(span_nodes, "A", { href: true, class: true });
      var a3_nodes = children(a3);
      t18 = claim_text(a3_nodes, "resume");
      a3_nodes.forEach(detach);
      t19 = claim_text(span_nodes, ". Any questions or comments can be\n			directed to me by filling in the blanks at\n			");
      a4 = claim_element(span_nodes, "A", { href: true, class: true });
      var a4_nodes = children(a4);
      t20 = claim_text(a4_nodes, "nathangk@st_nf_rd.edu");
      a4_nodes.forEach(detach);
      t21 = claim_text(span_nodes, ".");
      span_nodes.forEach(detach);
      t22 = claim_space(div0_nodes);
      p4 = claim_element(div0_nodes, "P", { id: true, class: true });
      var p4_nodes = children(p4);
      t23 = claim_text(p4_nodes, "things that might interest you");
      p4_nodes.forEach(detach);
      t24 = claim_space(div0_nodes);
      hr1 = claim_element(div0_nodes, "HR", { class: true, style: true });
      t25 = claim_space(div0_nodes);
      a5 = claim_element(div0_nodes, "A", { class: true, href: true });
      var a5_nodes = children(a5);
      t26 = claim_text(a5_nodes, "[Web App] Odd One Out: A Game of Words in Vector Semantics");
    ...
    h() {
      ...
      attr(img, "class", "ph2");
      if (!src_url_equal(img.src, img_src_value = base + "/favicon.svg"))
        attr(img, "src", img_src_value);
      attr(img, "alt", "logo");
      attr(p0, "id", "tit");
      attr(p0, "class", "f1-ns tc ma0 b flex items-center justify-center w-100 svelte-147asu5");
      attr(hr0, "class", "w-100 ma0 mt3");
      set_style(hr0, "border", "2px solid black");
      attr(p1, "class", "rainbow-text svelte-147asu5");
      attr(p2, "class", "rainbow-text svelte-147asu5");
      attr(p3, "class", "rainbow-text svelte-147asu5");
      attr(a0, "href", "https://github.com/nathankim7");
      attr(a0, "class", "svelte-147asu5");
      attr(a1, "href", "https://www.linkedin.com/in/nathan-kim-71b568155/");
      attr(a1, "class", "svelte-147asu5");
      attr(a2, "href", "https://scholar.google.com/citations?user=OQKjhFcAAAAJ&hl=en");
      attr(a2, "class", "svelte-147asu5");
      attr(a3, "href", base + "/files/Nathan_Kim_Resume.pdf");
      attr(a3, "class", "svelte-147asu5");
      attr(a4, "href", base);
      attr(a4, "class", "svelte-147asu5");
      set_style(span, "font-size", "1.3rem");
      set_style(span, "line-height", "1.3em");
      attr(span, "class", "ma0 mt3 w-100 content svelte-147asu5");
      attr(p4, "id", "tit2");
      attr(p4, "class", "f2-ns tc ma0 mt5 mb2 w-100 b svelte-147asu5");
      attr(hr1, "class", "w-100 ma0 mv2");
      set_style(hr1, "border", "2px solid black");
      attr(a5, "class", "mt2 f4 content svelte-147asu5");
      attr(a5, "href", "https://oddoneout.herokuapp.com/");
      attr(div0, "id", "content");
      attr(div0, "class", "flex flex-column flex-auto pa3 svelte-147asu5");
      attr(div1, "class", "flex justify-center w-100 h-100");
    },
    m(target, anchor2) {
      append_hydration(document.head, link);
      insert_hydration(target, t0, anchor2);
      insert_hydration(target, div1, anchor2);
      mount_component(ants, div1, null);
      append_hydration(div1, t1);
      append_hydration(div1, div0);
      append_hydration(div0, p0);
      append_hydration(p0, t2);
      append_hydration(p0, img);
      append_hydration(div0, t3);
      append_hydration(div0, hr0);
      append_hydration(div0, t4);
      append_hydration(div0, span);
      append_hydration(span, t5);
      append_hydration(span, p1);
      append_hydration(p1, t6);
      append_hydration(span, t7);
      append_hydration(span, p2);
      append_hydration(p2, t8);
      append_hydration(span, t9);
      append_hydration(span, p3);
      ...
      append_hydration(div0, t22);
      append_hydration(div0, p4);
      append_hydration(p4, t23);
      append_hydration(div0, t24);
      append_hydration(div0, hr1);
      append_hydration(div0, t25);
      append_hydration(div0, a5);
    ...
    i(local) {
      if (current)
        return;
      transition_in(ants.$$.fragment, local);
      current = true;
    },
    o(local) {
      transition_out(ants.$$.fragment, local);
      current = false;
    },
    d(detaching) {
      detach(link);
